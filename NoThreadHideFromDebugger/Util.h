...
typedef enum _SYSTEM_INFORMATION_CLASS
{
    ...
    SystemLoadGdiDriverInSystemSpace,      // s: SYSTEM_GDI_DRIVER_INFORMATION (kernel-mode only) (same as
    ...
    SystemProcessorIdleCycleTimeInformation, // q: SYSTEM_PROCESSOR_IDLE_CYCLE_TIME_INFORMATION[] (EX in: USHORT
                                             // ProcessorGroup)
    ...
    SystemSpecialPoolInformation,      // q; s: SYSTEM_SPECIAL_POOL_INFORMATION (requires SeDebugPrivilege) //
    ...
    SystemImageFileExecutionOptionsInformation, // s: SYSTEM_IMAGE_FILE_EXECUTION_OPTIONS_INFORMATION (requires
                                                // SeTcbPrivilege)
    SystemCoverageInformation, // q: COVERAGE_MODULES s: COVERAGE_MODULE_REQUEST // ExpCovQueryInformation (requires
                               // SeDebugPrivilege)
    SystemPrefetchPatchInformation,            // SYSTEM_PREFETCH_PATCH_INFORMATION
    SystemVerifierFaultsInformation,           // s: SYSTEM_VERIFIER_FAULTS_INFORMATION (requires SeDebugPrivilege)
    SystemSystemPartitionInformation,          // q: SYSTEM_SYSTEM_PARTITION_INFORMATION
    SystemSystemDiskInformation,               // q: SYSTEM_SYSTEM_DISK_INFORMATION
    SystemProcessorPerformanceDistribution,    // q: SYSTEM_PROCESSOR_PERFORMANCE_DISTRIBUTION (EX in: USHORT
                                               // ProcessorGroup) // 100
    ...
    SystemLogicalProcessorAndGroupInformation, // q: SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX (EX in:
                                               // LOGICAL_PROCESSOR_RELATIONSHIP
    // RelationshipType) // since WIN7 // KeQueryLogicalProcessorRelationship
    SystemProcessorCycleTimeInformation, // q: SYSTEM_PROCESSOR_CYCLE_TIME_INFORMATION[] (EX in: USHORT ProcessorGroup)
    SystemStoreInformation,              // q; s: SYSTEM_STORE_INFORMATION (requires SeProfileSingleProcessPrivilege) //
    ...
    SystemProcessorPerformanceInformationEx,   // q: SYSTEM_PROCESSOR_PERFORMANCE_INFORMATION_EX // (EX in: USHORT
                                               // ProcessorGroup) // since
    // WINBLUE
    ...
    SystemProcessorCycleStatsInformation, // q: SYSTEM_PROCESSOR_CYCLE_STATS_INFORMATION (EX in: USHORT ProcessorGroup)
                                          // // 160
    ...
    SystemCodeIntegrityPlatformManifestInformation, // q: SYSTEM_SECUREBOOT_PLATFORM_MANIFEST_INFORMATION // since
                                                    // REDSTONE
    SystemInterruptSteeringInformation,             // q: in: SYSTEM_INTERRUPT_STEERING_INFORMATION_INPUT, out:
    // SYSTEM_INTERRUPT_STEERING_INFORMATION_OUTPUT // NtQuerySystemInformationEx // 180
    ...
    SystemFeatureConfigurationInformation, // q: in: SYSTEM_FEATURE_CONFIGURATION_QUERY, out:
                                           // SYSTEM_FEATURE_CONFIGURATION_INFORMATION; s:
    // SYSTEM_FEATURE_CONFIGURATION_UPDATE // NtQuerySystemInformationEx // since 20H1 // 210
    SystemFeatureConfigurationSectionInformation, // q: in: SYSTEM_FEATURE_CONFIGURATION_SECTIONS_REQUEST, out:
    // SYSTEM_FEATURE_CONFIGURATION_SECTIONS_INFORMATION // NtQuerySystemInformationEx
    SystemFeatureUsageSubscriptionInformation, // q: SYSTEM_FEATURE_USAGE_SUBSCRIPTION_DETAILS; s:
    ...
    SystemBuildVersionInformation,             // q: in: ULONG (LayerNumber), out: SYSTEM_BUILD_VERSION_INFORMATION //
                                               // NtQuerySystemInformationEx //
    // 222
    ...
    SystemSupportedProcessorArchitectures2, // q: in opt: HANDLE, out:
                                            // SYSTEM_SUPPORTED_PROCESSOR_ARCHITECTURES_INFORMATION[] //
    // NtQuerySystemInformationEx // 230
    SystemSingleProcessorRelationshipInformation, // q: SYSTEM_LOGICAL_PROCESSOR_INFORMATION_EX // (EX in:
    ...
    SystemOriginalImageFeatureInformation, // q: in: SYSTEM_ORIGINAL_IMAGE_FEATURE_INFORMATION_INPUT, out:
    // SYSTEM_ORIGINAL_IMAGE_FEATURE_INFORMATION_OUTPUT // NtQuerySystemInformationEx
    SystemMemoryNumaInformation, // SYSTEM_MEMORY_NUMA_INFORMATION_INPUT, SYSTEM_MEMORY_NUMA_INFORMATION_OUTPUT
    SystemMemoryNumaPerformanceInformation, // SYSTEM_MEMORY_NUMA_PERFORMANCE_INFORMATION_INPUTSYSTEM_MEMORY_NUMA_PERFORMANCE_INFORMATION_INPUT,
...
extern "C"
{
    NTSTATUS PsGetProcessImageFileName(PEPROCESS Process);
    ...
    NTSTATUS PsGetProcessImageFileName(PEPROCESS Process);
    NTSTATUS ZwProtectVirtualMemory(HANDLE ProcessHandle, PVOID *BaseAddress, PSIZE_T RegionSize,
                                    ULONG NewAccessProtection, PULONG OldAccessProtection);
    NTSTATUS ZwAllocateVirtualMemory(HANDLE ProcessHandle, PVOID *BaseAddress, ULONG_PTR ZeroBits, PSIZE_T RegionSize,
                                     ULONG AllocationType, ULONG Protect);
    NTSTATUS ZwFreeVirtualMemory(HANDLE ProcessHandle, PVOID *BaseAddress, PSIZE_T RegionSize, ULONG FreeType);
    NTSTATUS ZwQuerySystemInformation(SYSTEM_INFORMATION_CLASS SystemInformationClass, PVOID SystemInformation,
...
void *GetKernelBase()
{
    ...
    Modules = (PRTL_PROCESS_MODULES)ExAllocatePoolWithTag(NonPagedPool, Bytes, 'ENRK');
    if (Modules == NULL)
    {
        return NULL;
    }
    ...
    {
        ExFreePoolWithTag(Modules, 'ENRK');
        return NULL;
    }
...
PIMAGE_SECTION_HEADER GetSectionHeader(uint64_t Base, const char *Section)
{
    if (!Base || !Section)
        return nullptr;
    ...
    const auto NumberOfSections = Nt->FileHeader.NumberOfSections;

    for (auto i = 0; i < NumberOfSections; ++i)
    {
        if (strstr((char *)pSection->Name, Section))
        {
            break;
        }
...
bool DataCompare(const char *Data, const char *bMask, const char *szMask)
{
...
uint64_t InternalFindPattern(uint64_t Base, uint64_t Size, const char *bMask, const char *szMask)
{
    for (auto i = 0ul; i < Size; ++i)
    {
        if (DataCompare(PCHAR(Base + i), bMask, szMask))
        {
            return Base + i;
        }
    }
...
uint64_t FindPattern(uint64_t Base, const char *Section, const char *bMask, const char *szMask)
{
    if (!Section || !bMask || !szMask)
        return 0;

    const auto pSection = GetSectionHeader(Base, Section);
    return pSection ? InternalFindPattern(Base + pSection->VirtualAddress, pSection->Misc.VirtualSize, bMask, szMask)
                    : 0;
}
